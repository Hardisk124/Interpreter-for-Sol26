...
import { loadConfig, runTest } from "./runner.js";
...
function writeResult(resultReport, outputFile) {
    /**
...
const DOUBLE_LETTER_SHORT_OPTION_NORMALIZATION = new Map([
    ["-ic", "--include-category"],
    ["-it", "--include-test"],
    ["-ec", "--exclude-category"],
    ["-et", "--exclude-test"],
...
function parseCliArgumentsRaw(argv) {
    return parseArgs({
...
function main() {
    ...
    const allUnexecuted = { ...unexecuted, ...filtered };
    //execute the tests, skip if dry-run
    const results = new Map();
    if (!args.dry_run) {
        for (const test of toRun) {
            const outcome = runTest(test, config);
            if (outcome.kind === "report") {
                results.set(test.name, { test, report: outcome.report });
            }
            else {
                allUnexecuted[test.name] = outcome.reason;
            }
        }
    }
    //build and write the report
    const report = generateReport(discovered, allUnexecuted, results, args.dry_run);
