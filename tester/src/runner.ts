 ...
 */
function execPrecess(bin: string, args: string[], stdinFile: string | null): ExecResult {
 ...
 */
function runParser(parserPath: string, sourcePath: string): ExecResult {
 ...
 */
function extractSourceCode(testFilePath: string): string {
 ...
 */
function writeTempSource(content: string): { path: string; cleanup: () => void } {
 ...
 */
function writeTempXml(content: string): { path: string; cleanup: () => void } {
 ...
 */
function runDiff(expected: string, actual: string): string | null {
...
/**
 * @brief result of running a test case
 */
export type RunResult =
  // test was executed (successful or failed)
  | { kind: "report"; report: TestCaseReport }
  // test was not executed due to some reason
  | { kind: "unexecuted"; reason: UnexecutedReason };
 ...
 */
export function runTest(testCase: TestCaseDefinition, config: RunnerConfig): RunResult {
 ...
 * @return RunResult object with the report of the test case
 */
function runParseOnly(testCase: TestCaseDefinition, config: RunnerConfig): RunResult {
  let parser: ExecResult;
  try {
    ...
    return {
      ...
      reason: new UnexecutedReason(
        ...
        `Failed to prepare parser input: ${message}`
      ),
    };
  }
  ...
  if (parser.failed) {
    return {
      ...
      reason: new UnexecutedReason(
        ...
        `Failed to execute parser: ${parser.stderr}`
      ),
    };
  }
 ...
 * @return RunResult object with the report of the test case
 */
function runExecuteOnly(testCase: TestCaseDefinition, config: RunnerConfig): RunResult {
  let interpreter: ExecResult;
  try {
    const sourceCode = extractSourceCode(testCase.test_source_path);
    const tempSource = writeTempSource(sourceCode);
    try {
    ...
    return {
      ...
      reason: new UnexecutedReason(
        UnexecutedReasonCode.OTHER,
        `Failed to prepare interpreter input: ${message}`
      ),
    };
  }
  ...
  if (interpreter.failed) {
    return {
      ...
      reason: new UnexecutedReason(
        UnexecutedReasonCode.CANNOT_EXECUTE,
        `Failed to execute interpreter: ${interpreter.stderr}`
      ),
    };
  }
 ...
 * @return RunResult object with the report of the test case
 */
function runCombined(testCase: TestCaseDefinition, config: RunnerConfig): RunResult {
  let parser: ExecResult;
  try {
    const sourceCode = extractSourceCode(testCase.test_source_path);
    const tempSource = writeTempSource(sourceCode);
...
function buildInterpreterReport(
