...
interface TestDataHeader {
  description: string | null; //***
  category: string | null; //+++
  points: number | null; // >>>
  sourceCode: string; //after first \n
  expectedParserCode: number[]; //!C!
  expectedInterpreterCode: number[]; //!I!
}
 ...
 */
function parseTestFile(filePath: string): TestDataHeader {
  const content = readFileSync(filePath, "utf-8");
  const lines = content.split(/\r?\n/);
  const data: TestDataHeader = {
    description: null,
    category: null,
    points: null,
    sourceCode: "",
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("***")) {
      ...
      if (!isNaN(pointsStr)) data.points = pointsStr;
    } else if (line.startsWith("!C! ")) {
      const code = parseInt(line.slice(4).trim(), 10);
      if (!isNaN(code)) data.expectedParserCode.push(code);
    } else if (line.startsWith("!I! ")) {
      const code = parseInt(line.slice(4).trim(), 10);
      if (!isNaN(code)) data.expectedInterpreterCode.push(code);
    }
    i++;
  }
 ...
 */
function determineTestCaseType(testData: TestDataHeader): TestCaseType | null {
  ...
  if (hasParserCodes && hasInterpreterCodes) return TestCaseType.COMBINED;
  if (hasParserCodes && !hasInterpreterCodes) return TestCaseType.PARSE_ONLY;
  if (!hasParserCodes && hasInterpreterCodes) return TestCaseType.EXECUTE_ONLY;
 ...
 */
export function discoverTests(testsDir: string, recursive: boolean): DiscoveryResult {
  ...
  for (const file of testFiles) {
    ...
    if (data.category === null) {
      unexecuted[name] = new UnexecutedReason(
        UnexecutedReasonCode.MALFORMED_TEST_CASE_FILE,
        `Missing required category in test file: ${file}`
      );
      continue;
    }
    ...
    if (data.points === null) {
      unexecuted[name] = new UnexecutedReason(
        UnexecutedReasonCode.MALFORMED_TEST_CASE_FILE,
        `Missing required points in test file: ${file}`
      );
      continue;
    }
    ...
    if (testType === null) {
      unexecuted[name] = new UnexecutedReason(
        ...
        `Cannot determine test case type for file: ${file}`
      );
      continue;
    }
    ...
    try {
      const def = new TestCaseDefinition({
        ...
        expected_parser_exit_codes:
          data.expectedParserCode.length > 0 ? data.expectedParserCode : null,
        expected_interpreter_exit_codes:
          data.expectedInterpreterCode.length > 0 ? data.expectedInterpreterCode : null,
      });
      discovered.push(def);
    } catch (err) {
